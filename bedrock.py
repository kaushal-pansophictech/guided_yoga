...
def capture_and_share():
    ...
    if st.button("Start Capture"):
        ...
        while time.time() - start_time < recording_time:
            ...
            frame, landmarks, segmentation_mask = detect_pose(frame, pose, draw_landmarks=True)
            frames.append(frame)
    ...
    if app_mode == "Welcome":
        ...
        if uploaded_file is not None:
            if uploaded_file.type in ["image/jpeg", "image/png", "image/jpg"]:
                ...
                image, landmarks, segmentation_mask = detect_pose(image, pose, draw_landmarks=True)
                if landmarks:
                    keypoints = extract_keypoints(landmarks)
                    detected_pose = identify_pose(keypoints)
